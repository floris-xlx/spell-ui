...
Three states: resting, hover, and active. The slight lift on hover tells the user "this is clickable." The press-down on active gives tactile feedback. Keep the transition under 200ms or it feels sluggish — this is the kind of [micro-interaction](/blog/micro-interactions-that-build-trust) that signals product quality.
...
A rule I follow: each card should answer one question. A product card answers "what is this and how much does it cost?" A blog card answers "what is this article about?" A profile card answers "who is this person?" The rest is noise that competes with [the visual hierarchy](/blog/visual-hierarchy-web-design) you're trying to establish.
...
**No image containment.** Images inside cards often bleed into the background when their colors are similar to the card surface. Add `outline: 1px solid rgba(0,0,0,0.1)` with `outline-offset: -1px` to images. This creates subtle visual containment without a hard border. In [dark mode](/blog/dark-mode-design-guide), swap to `rgba(255,255,255,0.1)`.
