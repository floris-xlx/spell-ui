...
Text animations are the highest-ROI visual improvement you can make to a landing page. A hero heading that fades in word-by-word instantly signals polish. Static text sitting there when the page loads signals "template." (For where these effects fit into the broader page, see our [hero section design best practices](/blog/hero-section-best-practices).)
...
A gradient that moves across text creates a shimmer or highlight effect. This works by setting the background to a moving linear gradient and clipping it to the text — our [shimmer text effect with CSS guide](/blog/shimmer-text-effect-css) breaks down each piece of the technique.
...
Animate text only when it scrolls into view using [IntersectionObserver](https://developer.mozilla.org/en-US/docs/Web/API/IntersectionObserver). Our [CSS scroll-triggered animations guide](/blog/css-animation-on-scroll) covers this pattern in more depth, including the newer `animation-timeline: view()` approach:
...
Whichever approach you use, keep these rules in mind — our [web animation performance guide](/blog/web-animation-performance) covers the rendering pipeline behind them:
