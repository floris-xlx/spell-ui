...
[WebGL](https://developer.mozilla.org/en-US/docs/Web/API/WebGL_API) backgrounds are what separate a "nice landing page" from one that makes people pause and look around. That animated gradient that feels alive, those light rays that shift with the cursor, the organic noise patterns that breathe — they're all WebGL under the hood. (For inspiration on where these effects fit, browse our roundup of [websites with great UI](/blog/websites-great-ui-inspiration).)
...
WebGL backgrounds can tank performance if you're not careful — our [web animation performance guide](/blog/web-animation-performance) covers the broader rules for staying at 60fps:
...
4. **Lazy load the canvas** — don't block initial page render with shader compilation. Load Three.js asynchronously (see our [lazy loading in React guide](/blog/lazy-loading-react) for the patterns)
...
Building custom WebGL backgrounds from scratch is rewarding but time-intensive. If you want the visual impact without the shader learning curve, there are pre-built options — our roundup of the [best React animation libraries](/blog/best-react-animation-libraries) is a good starting point.
