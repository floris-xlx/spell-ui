...
The infinite marquee — a continuous horizontal scroll of logos, testimonials, or text — is everywhere on modern landing pages. It signals social proof, adds visual motion, and fills space without demanding attention. (For more on the social-proof side specifically, see our guide on [adding social proof to your landing page](/blog/landing-page-social-proof).)
...
CSS `transform: translateX()` is GPU-composited, so the animation itself is cheap — our [web animation performance guide](/blog/web-animation-performance) explains why this property choice matters. But there are a few things to watch:

### Image Loading

If your marquee contains images (logos, avatars), they should be fully loaded before the animation starts. Images popping in mid-scroll looks broken. Our [image optimization for web guide](/blog/image-optimization-web) covers how to keep those assets light enough to load before they're needed.
...
If you want a marquee without building one, [Spell UI](https://spell.sh) has a `Marquee` component that covers duplication, direction, speed control, pause on hover, edge fading, reduced motion, and vertical mode. Install it via the CLI and drop it in. For more pre-built motion components in the same family, see our roundup of the [best React animation libraries](/blog/best-react-animation-libraries).
