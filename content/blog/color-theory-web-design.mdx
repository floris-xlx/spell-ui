...
In practice, most web interfaces use one primary hue, one neutral palette (grays), and one accent color. That's three colors, not twelve. Color is also one of the strongest tools you have for [building visual hierarchy](/blog/visual-hierarchy-web-design) on a page.
...
Use the 500-600 range for primary actions. The 100-200 range for backgrounds and badges. The 700-900 range for text on light backgrounds. These scales also become the foundation for any [design system you build later](/blog/design-system-guide).
...
Dark mode isn't "invert the colors." That approach produces eye-searing white text on pure black, which is harder to read than the light version. Our [dark mode design guide](/blog/dark-mode-design-guide) covers the full set of decisions involved, but the color-specific ones are below.
...
If you're starting from scratch, pick one blue between `hsl(220, 80%, 50%)` and `hsl(230, 70%, 55%)`. Pair it with Tailwind's default gray scale (we walk through this stack in our [Tailwind design system guide](/blog/tailwind-design-system)). Add a green for success, red for errors, yellow for warnings.
