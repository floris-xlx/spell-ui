...
The single most important performance advice: do not optimize blindly. [React](https://react.dev) is fast by default. Most perceived performance problems come from a handful of specific patterns, and fixing the wrong thing wastes time while the real bottleneck persists. If you are optimizing a production app, pair this guide with our [Core Web Vitals guide](/blog/core-web-vitals-guide) to make sure you are measuring the metrics that actually matter.
...
The `SettingsPanel` code is only downloaded when the user clicks the button. For large applications, this can reduce the initial bundle by 30-50%. For a deeper look at this technique, see our complete guide to [lazy loading and code splitting in React](/blog/lazy-loading-react).
...
- **Debounce rapid updates.** Typing in a search field should not trigger a re-render on every keystroke. Debounce the state update — a small `useDebounce` helper from our [practical guide to React custom hooks](/blog/react-custom-hooks-guide) handles this in a few lines.

Performance optimization in React is not about applying every technique everywhere. It is about understanding your specific bottlenecks and applying the right tool to each one. If you are running on the App Router, our [Next.js performance tips](/blog/nextjs-performance-tips) cover the framework-level concerns that sit above these techniques.
