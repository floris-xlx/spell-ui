...
If you've ever tried to style markdown or CMS-rendered content with [Tailwind CSS](https://tailwindcss.com), you've hit the problem: Tailwind's reset strips all default browser styles. Headings, paragraphs, lists, and blockquotes all render as unstyled text. (For background on the typographic decisions baked into the plugin's defaults, see our [typography best practices for web design](/blog/typography-web-design).)
...
The Typography plugin works with Tailwind's dark mode out of the box — see our [Tailwind dark mode setup guide](/blog/tailwind-dark-mode-guide) if you haven't wired up the `dark` class strategy yet. Prefix prose modifiers with `dark:` to swap color schemes:
...
Sometimes the prose defaults don't match your design. If you're maintaining a [Tailwind design system](/blog/tailwind-design-system), the override layer should mirror your design tokens rather than introduce new values. You can override any element's styles in the Tailwind config:
...
- **Tables** are styled but not made responsive. Wrap tables in a `div` with `overflow-x-auto` for horizontal scrolling on mobile — see our [responsive design with Tailwind CSS guide](/blog/tailwind-responsive-design) for more patterns like this.
