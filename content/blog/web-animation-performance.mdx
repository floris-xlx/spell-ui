...
Smooth animation is 60 frames per second. That gives you 16.6 milliseconds per frame to do everything — run JavaScript, calculate styles, perform layout, paint pixels, and composite layers. Miss that budget and you get jank. (Frame-budget thinking is also central to broader page performance — see our [Core Web Vitals guide](/blog/core-web-vitals-guide) for how this fits into the metrics Google measures.)
...
For simple state transitions (hover effects, enter/exit animations, loading states), CSS animations and transitions are ideal. The browser can optimize them and run them off the main thread in many cases. Our roundup of [creative CSS hover effects for buttons](/blog/css-hover-effects-buttons) and our guide to [building custom animations with Tailwind CSS](/blog/tailwind-custom-animations) cover the patterns that fit this category.
...
For a comparison of which library to reach for in each of those situations, see our roundup of the [best React animation libraries](/blog/best-react-animation-libraries) and our [practical Motion guide](/blog/framer-motion-guide).
...
- Test on a real mid-range phone, not just your development machine — see our [website loading speed guide](/blog/website-loading-speed) for more on real-device performance testing
