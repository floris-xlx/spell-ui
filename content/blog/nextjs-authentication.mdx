...
Authentication is the feature every app needs and nobody enjoys building. [Next.js](https://nextjs.org) doesn't ship with auth built in, but [Auth.js](https://authjs.dev) (formerly NextAuth.js) fills that gap well. Here's how to set it up properly with the App Router, protect routes with middleware, and handle sessions on both the server and client. If you haven't worked with the App Router yet, our [App Router guide](/blog/nextjs-app-router-guide) is a useful primer.
...
If route handlers are new to you, our guide on [Next.js API routes and Server Actions](/blog/nextjs-api-routes) explains the conventions in depth.
...
1. **Always use HTTPS in production.** Auth cookies over HTTP are vulnerable to interception. Our [Next.js deployment guide](/blog/nextjs-deployment-guide) walks through HTTPS setup on Vercel, Docker, and self-hosted environments.
