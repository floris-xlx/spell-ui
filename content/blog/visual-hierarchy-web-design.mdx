...
For headings, use `text-wrap: balance` to distribute text evenly across lines. This prevents orphaned words sitting alone on the last line of a heading, which disrupts the visual weight of the block. (More on heading-level typography decisions in our [typography best practices guide](/blog/typography-web-design).)
...
Use this deliberately. Your [primary call-to-action](/blog/landing-page-cta-best-practices) should be the most saturated, highest-contrast element on the screen. Secondary actions get muted colors. Tertiary actions can be text links. The [color theory fundamentals](/blog/color-theory-web-design) behind this — saturation, contrast ratio, accent reservation — apply to the whole page, not just the button.
...
When spacing is inconsistent, users can't tell where one group ends and another begins. They have to read to understand the structure instead of seeing it. Codifying these spacing values into a [design system](/blog/design-system-guide) is what keeps hierarchy consistent across an entire product.
