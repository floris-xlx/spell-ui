...
[Next.js](https://nextjs.org) gives you two ways to run server-side logic: Route Handlers (the successor to API routes) and Server Actions. They solve different problems, and knowing when to use each will save you from writing unnecessary code. Both are App Router features — if you're new to it, start with our [Next.js App Router guide](/blog/nextjs-app-router-guide) for the bigger picture.
...
- Auth provider callbacks — see our guide on [authentication in Next.js](/blog/nextjs-authentication) for a complete setup
...
Server Actions cut boilerplate. Instead of creating an API route, writing a fetch call, handling the response, and managing loading state, you define a function and call it. For internal mutations, they're almost always the better choice. Where these files live matters too — our piece on [structuring a Next.js project for scale](/blog/nextjs-project-structure) shows good conventions for organizing actions and route handlers.
...
Pick the right tool for the job. Server Actions for your own UI mutations. Route Handlers for everything else. Both run on the server, both have full access to your backend — the difference is in how they're invoked and who consumes them. When you're ready to ship, see our [Next.js deployment guide](/blog/nextjs-deployment-guide) for getting these endpoints into production.
