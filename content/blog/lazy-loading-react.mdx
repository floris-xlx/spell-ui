...
Lazy loading means deferring the download and execution of code until it's actually needed. In a [React](https://react.dev) application, this translates directly to faster initial page loads — your user downloads only the code for what they can see, not the entire application. It's one of the highest-impact optimizations covered in our broader guide on [improving website loading speed](/blog/website-loading-speed).
...
[Next.js](https://nextjs.org) handles this automatically. Every file in the `app/` or `pages/` directory is its own chunk — no configuration needed. Code splitting is just one of the techniques in our [10 Next.js performance tips](/blog/nextjs-performance-tips) for production apps.
...
**Below-the-fold content.** Anything the user needs to scroll to see is a candidate. The same logic applies to media assets — see our guide on [optimizing images for web performance](/blog/image-optimization-web) for native lazy loading patterns.
...
Start with route-based splitting, then target heavy components, and preload anything the user is likely to need next. Track the impact in real terms by watching your [Core Web Vitals](/blog/core-web-vitals-guide) — code splitting moves LCP and INP more than almost any other change.
