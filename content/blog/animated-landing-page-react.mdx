...
The [hero section](/blog/hero-section-best-practices) is the first thing visitors see, so it's the highest-impact place to add animation. A simple fade-up entrance on the headline, subtext, and CTA creates a sense of craftsmanship that static text doesn't have.
...
Elements below the fold should animate in as the user scrolls to them. This transforms a wall of content into a guided experience. (For the underlying CSS techniques, see our [scroll-triggered animations guide](/blog/css-animation-on-scroll).)
...
Beyond entrance animations, [small interactive details](/blog/micro-interactions-that-build-trust) make the page feel alive:
...
**[Motion](https://motion.dev)** is the most popular React animation library. It provides declarative animation props, gesture handling, and layout animations. It's full-featured but adds bundle size — roughly 30-40KB gzipped. Our [practical Motion guide](/blog/framer-motion-guide) covers the API in depth, and there's a [broader comparison of React animation libraries](/blog/best-react-animation-libraries) if you're still picking.
...
Animation that causes jank destroys the polish it's supposed to create. Our [web animation performance guide](/blog/web-animation-performance) goes deeper, but the essential rules:
