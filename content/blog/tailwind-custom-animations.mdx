...
[Tailwind CSS](https://tailwindcss.com) ships with a handful of built-in animations — `animate-spin`, `animate-pulse`, `animate-ping`, and `animate-bounce`. They cover the basics, but real-world projects need more. Fade-ins, slide-ups, scale effects, shimmer loaders — these require custom keyframes. (For animations that go beyond what CSS can handle on its own, our roundup of the [best React animation libraries](/blog/best-react-animation-libraries) covers when to reach for a JS-based tool.)
...
Skeleton loaders with a shimmer effect are a common pattern — and the same technique extends to text gradients, as covered in our [shimmer text effect with CSS guide](/blog/shimmer-text-effect-css). Here's how to build one entirely in Tailwind config:
...
For lists or grids where items should animate in sequentially, combine `animate-slide-up` with `animation-delay` via inline styles or CSS variables. To trigger them as the user scrolls, pair this with the techniques in our [CSS scroll-triggered animations guide](/blog/css-animation-on-scroll):
...
A few rules to keep your animations smooth — our [web animation performance guide](/blog/web-animation-performance) digs into the rendering pipeline behind these:
