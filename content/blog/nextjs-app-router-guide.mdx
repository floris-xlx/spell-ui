...
The App Router is the biggest architectural shift in [Next.js](https://nextjs.org) history. It replaced the Pages Router's flat, page-centric model with a nested, layout-driven approach built on React Server Components. If you've been using Next.js for a while, some patterns feel unfamiliar. If you're starting fresh, our [Getting Started with Next.js guide](/blog/getting-started-nextjs) covers the basics first — you're in the right place.
...
This works because the App Router streams HTML using React's Suspense architecture. The layout renders immediately, the loading state shows, and the page content replaces it once data is ready. No client-side loading spinners or skeleton management required. Streaming is one of several techniques we cover in our [10 Next.js performance tips](/blog/nextjs-performance-tips) for production apps.
...
This lets you apply different layouts to different sections of your app without polluting the URL structure. Marketing pages get a public header and footer. App pages get a sidebar and nav bar. For a fuller picture of how to organize routes alongside features and shared code, see our guide on [structuring a Next.js project for scale](/blog/nextjs-project-structure).
...
If you're migrating an existing project, you can adopt incrementally — both routers work simultaneously. Move routes one at a time, starting with the simplest pages, and work your way up to the more complex ones. Once you're comfortable with the routing model, our guide to [Next.js API routes and Server Actions](/blog/nextjs-api-routes) covers the server-side companion patterns.
