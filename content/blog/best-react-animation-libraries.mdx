...
The most popular React animation library for good reason. Declarative API, layout animations, gesture support, exit animations with `AnimatePresence`. If you're building a typical React app with route transitions and interactive components, [Motion](https://motion.dev) is the safe bet — our [practical guide to Motion](/blog/framer-motion-guide) covers the patterns you'll actually use day to day.
...
Tailwind's built-in animation utilities (`animate-spin`, `animate-pulse`, `animate-bounce`) plus custom keyframes cover a surprising amount of ground — our guide on [building custom animations with Tailwind CSS](/blog/tailwind-custom-animations) walks through extending the system properly.
...
Smooth scroll library that pairs well with any animation tool. Handles inertia scrolling and scroll-triggered effects. For the native CSS counterpart, see our [CSS scroll-triggered animations guide](/blog/css-animation-on-scroll).
...
3D animations and [WebGL](https://developer.mozilla.org/en-US/docs/Web/API/WebGL_API) effects in React. React Three Fiber gives you a declarative API over Three.js. Overkill for UI animation, essential for 3D — our [guide to WebGL backgrounds in React](/blog/webgl-backgrounds-react) shows a practical setup.
...
Don't overcomplicate it. Most projects need one library, maybe two. Start with the simplest tool that solves your problem and upgrade when you hit its limits. Whatever you pick, our [web animation performance guide](/blog/web-animation-performance) covers the rendering rules that keep things at 60fps.
