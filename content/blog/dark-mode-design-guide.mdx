...
Create depth by layering progressively lighter grays. In light mode, elevation is communicated through shadows. In dark mode, elevation is communicated through surface lightness — higher elements are lighter. This is the same principle that drives [visual hierarchy on the page itself](/blog/visual-hierarchy-web-design): contrast between layers signals importance.
...
Saturated colors that look great on white can overwhelm on dark surfaces. A vibrant blue button that works in light mode looks like a neon sign in dark mode. (Our [color theory breakdown](/blog/color-theory-web-design) covers why saturation behaves differently against dark backgrounds.)
...
Respect the operating system preference as the default, then let users override it. If you're on Tailwind, the [dark mode setup guide](/blog/tailwind-dark-mode-guide) walks through wiring this up cleanly.
...
Dark mode is a second design system, not a CSS trick. Treat it with the same care as your light theme, and structure both around the same [design system foundations](/blog/design-system-guide), and users will actually prefer it.
