...
Performance comes from dozens of small decisions during development, not a last-minute audit. [Next.js](https://nextjs.org) handles a lot of optimization automatically, but a default Next.js app and a properly tuned one differ substantially. The metrics that matter most are covered in our [Core Web Vitals guide](/blog/core-web-vitals-guide) — read that first if you want to know what you're optimizing for.
...
For format choices, srcset patterns, and CDN delivery beyond what `next/image` does, see our complete guide on [optimizing images for web performance](/blog/image-optimization-web).
...
For font subsetting, variable fonts, and metric overrides that minimize layout shift, see our deeper dive on [web fonts and performance](/blog/web-fonts-performance).
...
We take this approach with [Spell UI](https://spell.sh) components too — heavier animation components that depend on WebGL or complex motion libraries lazy-load by default so they don't block your initial page render. The component loads when the user scrolls to it, not when the page first loads. For the patterns behind this — including IntersectionObserver-based loading and preloading on hover — see our [complete guide to lazy loading in React](/blog/lazy-loading-react).
...
Ship less JavaScript, load things when they're needed, cache aggressively, and measure regularly. These ten tips cover most performance wins in a Next.js app. To validate everything once you're done, run through our [Lighthouse score guide](/blog/lighthouse-score-guide).
