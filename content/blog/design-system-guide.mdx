...
Design tokens are the atomic values your entire system is built on. [Colors](/blog/color-theory-web-design), spacing, [typography](/blog/typography-web-design), border radii, shadows. Define these first and everything downstream inherits consistency.
...
Structure your tokens in two layers. **Global tokens** define raw values (colors, sizes). **Semantic tokens** map those values to purposes (`--color-text-primary`, `--color-bg-surface`). Semantic tokens make theming possible — swap the mappings for [dark mode](/blog/dark-mode-design-guide), and every component adapts automatically.
...
2. **[Input](/blog/react-input-components)** — Text, select, checkbox, radio. Error and disabled states.
3. **[Card](/blog/ui-card-design-patterns)** — Container with consistent padding, radius, and shadow.
4. **Badge** — Status indicators and tags.
5. **[Modal / Dialog](/blog/react-modal-dialog)** — Overlay with focus trapping and escape handling.
...
The common advice is "don't build a design system until you need one." I'd refine that: don't build a design system until you have at least two products sharing components, or one product large enough that different teams are building the same patterns independently. If you're working in Tailwind, our [Tailwind design system guide](/blog/tailwind-design-system) covers the implementation specifics.
