...
Sidebars are a staple of dashboard layouts. They seem simple until you need to handle collapsing, mobile viewports, keyboard navigation, and persistent state. Here's a practical approach to building one that handles all of these without over-engineering. The sidebar usually sits next to a [responsive navbar](/blog/react-navbar-tutorial), so the two patterns are worth thinking about together.
...
Second, close the mobile drawer when pressing Escape — the same Escape behavior every overlay should respect, including [accessible modal dialogs](/blog/react-modal-dialog):
...
The lazy initializer in `useState` avoids a hydration mismatch if you're using server-side rendering. The state reads from `localStorage` only on the client. If you reach for this pattern often, our [React custom hooks guide](/blog/react-custom-hooks-guide) shows how to wrap it in a reusable `useLocalStorage` hook.
