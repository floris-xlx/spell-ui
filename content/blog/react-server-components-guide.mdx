...
[React](https://react.dev) Server Components (RSC) change where your components run, how data flows through your application, and what gets sent to the browser. Understanding the mental model behind RSC matters more than memorizing the API. RSC is the foundation of the [Next.js App Router](/blog/nextjs-app-router-guide), so the two concepts go hand in hand.
...
No loading spinners, no waterfall requests, no client-side caching to manage. The data is available when the component renders. For mutations and server-side writes, see how this pairs with [Next.js API routes and server actions](/blog/nextjs-api-routes).
...
- Custom hooks that use any of the above — most of the [practical custom hooks we cover](/blog/react-custom-hooks-guide) rely on browser APIs and must live in Client Components
...
Server Components are not a replacement for everything you know about React. They are a new tool for a specific class of problems: getting data from the server to the UI with less JavaScript, less latency, and less complexity. For the broader optimization picture once your tree is split correctly, our [React performance optimization guide](/blog/react-performance-optimization) covers the client-side techniques that still apply.
