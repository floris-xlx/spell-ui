...
Dark mode has gone from a nice-to-have to an expectation. Users want it, and [Tailwind CSS](https://tailwindcss.com) makes it straightforward to implement. But there are a few decisions to make early on that will save you from refactoring later. (For the design-side perspective on what makes a dark theme actually work, see our [dark mode design best practices](/blog/dark-mode-design-guide).)
...
The biggest dark mode mistake I see is scattering one-off color values throughout your markup. This is where a proper [Tailwind design system](/blog/tailwind-design-system) pays for itself — semantic color tokens defined as CSS custom properties keep both themes in sync:
...
Now you write `bg-background text-foreground` once, and it works in both themes. No `dark:` prefix needed on every element. You only use `dark:` for the occasional edge case where light and dark layouts genuinely differ. The same token approach works for typography — see our [Tailwind Typography plugin guide](/blog/tailwind-typography-plugin) for how `prose-invert` plugs into this same model.
...
- **Contrast ratios** — Test text against backgrounds in both modes. WCAG AA requires at least 4.5:1 for body text. Our [color theory for web design guide](/blog/color-theory-web-design) covers how to pick palettes that meet contrast targets in both modes.
