...
Scroll-driven animations (`animation-timeline`, `view()`, `scroll()`) are supported in Chrome, Edge, and Firefox. Safari added support in late 2024. For production apps that need to support older browsers, use IntersectionObserver as a fallback — Motion's `useInView` hook offers a comparable React-friendly API, covered in our [Motion guide](/blog/framer-motion-guide).
...
1. **Stick to `transform` and `opacity`** — these are composited by the GPU and won't trigger layout or paint. Our [web animation performance guide](/blog/web-animation-performance) digs into the rendering pipeline behind this rule
