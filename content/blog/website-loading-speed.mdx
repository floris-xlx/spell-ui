...
A slow website costs you users. 53% of mobile visitors abandon a page that takes more than 3 seconds to load. Most performance problems have straightforward fixes — and they map directly to the metrics in our [Core Web Vitals guide](/blog/core-web-vitals-guide).
...
The key insight: don't split everything. Splitting adds a network request. Only split components that are large (50KB+), conditionally rendered, or below the fold. Our complete guide to [lazy loading in React](/blog/lazy-loading-react) covers route-based and component-level patterns in more depth.
...
For the deeper version of this — `font-display` modes, fallback metric overrides, and Next.js font optimization — see our [web fonts and performance guide](/blog/web-fonts-performance).
...
7. Lazy load heavy components and below-the-fold images — combine with our guide on [optimizing images for web performance](/blog/image-optimization-web)

Each step compounds. A site that implements all of these typically loads 2-4x faster than the same site without them. Validate the result by running through our [Lighthouse score guide](/blog/lighthouse-score-guide).
