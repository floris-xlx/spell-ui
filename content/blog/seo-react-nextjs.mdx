...
[Next.js](https://nextjs.org) solves the rendering problem. But rendering is only part of SEO. Here's everything else you need to get right. For a deeper, Next.js-specific tour of the same topics, see our [complete Next.js SEO guide](/blog/nextjs-seo-guide). New to the framework entirely? Start with [Getting Started with Next.js](/blog/getting-started-nextjs).
...
**React Server Components (RSC)** — The default in the [App Router](/blog/nextjs-app-router-guide). Components render on the server with zero client-side JavaScript unless you explicitly add interactivity with `"use client"`.
...
SEO in Next.js is straightforward — it just requires covering each of these areas deliberately rather than assuming the framework handles it all. Search ranking is also tied to performance, so pair this checklist with our [Core Web Vitals guide](/blog/core-web-vitals-guide) and the [Lighthouse score guide](/blog/lighthouse-score-guide).
